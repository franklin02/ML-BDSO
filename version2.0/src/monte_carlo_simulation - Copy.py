...
def run_monte_carlo(bayesian_results, num_simulations=1000):
    ...
    # Generate Monte Carlo simulation data based on Bayesian priors
    for i in range(num_simulations):
        ...
        # Debug: Log each simulation's result for traceability
        if i < 5:  # Log only the first few to reduce verbosity
            logging.debug("Simulation %d, Result: %s", i + 1, run_result)

    # Calculate high-risk defect probabilities and their average
    high_risk_probs = [
    ...
    avg_high_risk_prob = np.mean(high_risk_probs) if high_risk_probs else 0.0001
    logging.info("Average high-risk defect probability: %.4f (dimensionless)", avg_high_risk_prob)

    # Plot and save the defect probability distribution histogram
    ...
    # Plot and save the trend of defect probabilities over simulations
    plt.figure(figsize=(10, 6))
    ...
    # Construct defect_data for sensitivity analysis
    defect_data = {
        "defect_probability": defect_probabilities
    }

    # Debug: Log the structure and contents of defect_data
    logging.debug("Defect Data Structure: %s, Sample Content: %s", type(defect_data),
                  defect_data['defect_probability'][:3])
