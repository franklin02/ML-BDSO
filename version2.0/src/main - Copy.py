...
def run_ml_bdso(new_data=None):
    ...
    logging.debug("Monte Carlo Results Structure: %s", type(monte_carlo_results))
    valid_results = []
    for run in monte_carlo_results:
        if isinstance(run, dict) and all(isinstance(k, str) and isinstance(v, (int, float)) for k, v in run.items()):
            valid_results.append(run)
        else:
            logging.error("Unexpected structure in monte_carlo_results: %s", run)
    ...
    # Prepare defect_data for sensitivity analysis
    defect_probabilities = []  # Initialize an empty list to store defect probabilities
    for run in valid_results:
        # Calculate the average defect probability for each run and append to the list
        avg_prob = np.mean([v for v in run.values() if not np.isnan(v)])
        defect_probabilities.append(avg_prob)
    defect_data = {"defect_probability": defect_probabilities}  # Assign the list to defect_data

    logging.debug("Prepared defect_data for sensitivity analysis: %s", type(defect_data))
    logging.debug("Sample defect_data (first entry): %s", valid_results[0] if valid_results else "No valid data")
